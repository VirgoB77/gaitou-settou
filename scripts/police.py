...
# 制度で分けない。読めている語まで「見に行っていない」に落ちる。
#
#   unresolved … 語彙を足せば減る
#   unobserved … **取りに行けば手に入る。ページでも、新しい版でも同じ**
#   undecided  … 正本が決めないと減らない
#   gone       … 時間が決める
#
# 「ページか版か」で分けない。`柏木町5丁目` は語も読めていて出どころも
# 取りに行っているが、**引いた境界データの版が古い**。
# 新しい年次を取れば手に入るので `unobserved`（2026-09-19・統括の判断）。
# **下敷きにするデータにも版がある。** 古い版で引けなかったのは
# 「無い」ではなく「その版には無い」。
#
...
def match(row, bd):
    ...
    # 語は読めていて、出どころも取りに行っている。**版が古いだけ。**
    # 新しい年次を取れば手に入るので unobserved。語彙では減らない。
    return None, hazure("e-Statに該当する町丁目がない", "unobserved"), key
